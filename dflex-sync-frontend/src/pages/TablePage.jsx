...
// =====================
// PARTIDA desde Supabase Portones
// =====================

// Intenta leer Portones (con comillas) y si falla prueba portones (sin comillas).
async function fetchPortonesPartidaByNvs(nvs) {
  if (!supabasePool) throw new Error('SUPABASE_DB_URL no está configurado');

  const clean = (Array.isArray(nvs) ? nvs : [])
    .map((x) => parseInt(x, 10))
  ...
  const tryQueries = [
    `SELECT nv, partida FROM "Portones" WHERE nv = ANY($1::int[])`,
    `SELECT nv, partida FROM portones WHERE nv = ANY($1::int[])`,
  ];
  ...
  for (const q of tryQueries) {
    try {
      const r = await supabasePool.query(q, [clean]);
      rows = r.rows || [];
      break;
    } catch (e) {
      continue;
  ...
  if (rows === null) {
    throw new Error('No se pudo consultar la tabla Portones/portones en Supabase.');
  }

  const map = new Map();
  for (const r of rows) {
    const nv = parseInt(r.nv, 10);
    if (!Number.isFinite(nv)) continue;
    const partida = r.partida !== null && r.partida !== undefined ? String(r.partida) : '';
    map.set(String(nv), partida);
  }
  return map;
}

async function applyPartidaFromPortones(rows) {
  ...
  const partidaByNv = await fetchPortonesPartidaByNvs(nvs);

  return list.map((r) => {
    const nv = parseInt(r?.NV ?? r?.nv, 10);
    const key = Number.isFinite(nv) ? String(nv) : null;
    const p = key ? (partidaByNv.get(key) || '') : '';

    const fallback =
      r?.partida ??
      r?.PARTIDA ??
      r?.Partida ??
      (r?.data && (r.data.partida || r.data.PARTIDA)) ??
      '';

    const finalPartida = p || (fallback ? String(fallback) : '');
...
async function getPreProduccionSqlRowsFromSupabase({ nv, partida } = {}) {
  ...
  // ✅ FILTRAR PARTIDA por Portones.partida (NO por data JSON)
  if (partida) {
    params.push(String(partida).trim());
    where.push(`
      nv IN (
        SELECT nv FROM "Portones" WHERE partida = $${params.length}::numeric
        UNION
        SELECT nv FROM portones WHERE partida = $${params.length}::numeric
      )
    `);
  ...
  // ✅ aplicar partida desde Portones en la salida
  return await applyPartidaFromPortones(base);
}
...
async function getPreProduccionValoresRows({ nv, partida } = {}) {
  ...
  // 1) base cruda (ya viene con PARTIDA/partida aplicado)
  ...
  // ✅ FORZAR PARTIDA DESDE PORTONES (por si algún overlay la pisa)
  return await applyPartidaFromPortones(merged);
}
...
// Soporta: ?nv= y/o ?partida=   (PARTIDA se filtra por Portones.partida)
